...
import { ZAC_AUGMENTS } from '../../src/augments/champions/zac';
 ...
 */
describe('Zac augments (M4)', () => {
  it('are all gated to Zac and nowhere else', () => {
    for (const a of ZAC_AUGMENTS) {
      expect(a.champion, a.id).toBe('zac');
      expect(augmentFitsChampion(a, 'zac'), a.id).toBe(true);
  ...
  it('has unique ids that cannot collide with another champion set', () => {
    const ids = ZAC_AUGMENTS.map((a) => a.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const id of ids) expect(id.startsWith('zac_'), id).toBe(true);
  });
  ...
  it('covers at least four build lanes with several picks each', () => {
    ...
    expect(ZAC_AUGMENTS.length).toBeGreaterThanOrEqual(8);
    const lanes = new Set(ZAC_AUGMENTS.flatMap((a) => a.tags ?? []));
    expect(lanes.size).toBeGreaterThanOrEqual(2);
  });

  it('spreads across tiers rather than sitting in one', () => {
    const tiers = new Set(ZAC_AUGMENTS.map((a) => a.tier));
