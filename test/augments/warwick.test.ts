...
import { WARWICK_AUGMENTS } from '../../src/augments/champions/warwick';
 ...
 */
describe('Warwick augments (M4)', () => {
  it('are all gated to Warwick and nowhere else', () => {
    for (const a of WARWICK_AUGMENTS) {
      expect(a.champion, a.id).toBe('warwick');
      expect(augmentFitsChampion(a, 'warwick'), a.id).toBe(true);
  ...
  it('has unique ids that cannot collide with another champion set', () => {
    const ids = WARWICK_AUGMENTS.map((a) => a.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const id of ids) expect(id.startsWith('ww_'), id).toBe(true);
  });
  ...
  it('covers at least four build lanes with several picks each', () => {
    ...
    expect(WARWICK_AUGMENTS.length).toBeGreaterThanOrEqual(8);
    const lanes = new Set(WARWICK_AUGMENTS.flatMap((a) => a.tags ?? []));
    expect(lanes.size).toBeGreaterThanOrEqual(2);
  });

  it('spreads across tiers rather than sitting in one', () => {
    const tiers = new Set(WARWICK_AUGMENTS.map((a) => a.tier));
